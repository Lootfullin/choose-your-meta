...
- **Стабильная принадлежность к коллекциям** — полное сканирование сохраняет TMDB ID и название коллекции у фильма, поэтому TMDb Box Sets не удаляет локализованные коллекции как пустые
- **Безопасная идентификация коллекций** — автоматическое сопоставление требует одинакового TMDB ID коллекции минимум у двух входящих фильмов; одноимённые чужие коллекции не выбираются по первому результату поиска
- **Безопасная идентификация фильмов и сериалов** — точный IMDb исправляет устаревший TMDB ID, а неоднозначный поиск по названию и году пропускается без назначения случайного результата
- **Самовосстановление коллекций** — фоновая сверка возвращает русское название после задач Jellyfin/TMDB, не затрагивая изображения
...
- **Надёжное сохранение настроек** — языковые приоритеты изображений корректно проходят полный цикл загрузки и сохранения Jellyfin
...
- Название фильма теперь важнее точного совпадения года при выборе результата TMDB
- Результаты с совершенно непохожим названием полностью отклоняются
- Разница между годом производства и годом премьеры в один год считается допустимой
- Исправлено ложное распознавание «Кавказской пленницы» как конкурса «Евровидение 1966»
