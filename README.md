...
- **🇷🇺 Русские названия и описания** — фильмы, сериалы, сезоны и эпизоды отображаются на русском языке
- **Русские данные о фильме** — слоган, жанры, студии, актёры, режиссёры и сценаристы
- **Единый состав фильмов и сериалов** — актёры сериалов и сезонов сопоставляются по TMDB ID и используют то же каноническое имя, что и фильмы
...
3. Нажмите **Настроить библиотеки автоматически**. Плагин включит себя для
   фильмов, сериалов, сезонов, эпизодов и коллекций и поставит выше TheMovieDB, сохранив
...
├── RussianSeasonProvider.cs      # Провайдер метаданных для сезонов
├── RussianEpisodeProvider.cs     # Провайдер метаданных для эпизодов
├── TmdbPeopleLocalization.cs     # Общие русские имена людей по TMDB ID
...
| `IRemoteMetadataProvider<Season, SeasonInfo>` | Удалённый провайдер для сезонов |
...
| `ICustomMetadataProvider<Season>` | Применяет русское название сезона после стандартного слияния Jellyfin |
| `ICustomMetadataProvider<Episode>` | То же для эпизодов |

## Решение проблем

1. **Проверьте логи Jellyfin** — ищите записи с префиксом `RussianMetadata:`, `RussianMetadata (Series):`, `ChooseYourMeta (Season):`, `RussianMetadata (Episode):`
...
- Исправлена потеря русских имён актёров и съёмочной группы при массовом сканировании
- Запросы русских имён выполняются последовательно и небольшими пакетами, чтобы не перегружать Wikidata
- Для временных ответов 429/502/503/504 добавлены автоматические повторные попытки
- Временный сетевой сбой больше не кэшируется как отсутствие русского имени
- Отрицательный кэш ограничен 15 минутами, после чего имя проверяется снова

### v1.4.0

- Добавлены русские названия и описания сезонов
- Добавлен актёрский состав и съёмочная группа сериалов и отдельных сезонов
- Фильмы, сериалы и сезоны используют единое каноническое имя человека по TMDB ID
- Русское имя берётся из Wikidata, английское используется только при отсутствии русского
- Добавлен общий кэш имён людей, исключающий повторные запросы для каждого сезона
- Автоматическая настройка TV-библиотек теперь включает провайдер для `Season`
